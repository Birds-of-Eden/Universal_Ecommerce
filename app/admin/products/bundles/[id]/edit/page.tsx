...
import { ArrowLeft, Save, Package, DollarSign, Upload } from "lucide-react";
    ...
    const fetchBundle = async () => {
      try {
        ...
        setFormData({
          ...
          categoryId: bundleData.categoryId.toString(),
          brandId: bundleData.brandId?.toString() || "none",
          image: bundleData.image || "",
          gallery: bundleData.gallery || [],
          available: bundleData.available,
          featured: bundleData.featured,
          currency: bundleData.currency,
        });

        // Set bundle items
        const items: BundleItem[] = bundleData.bundleItems.map(item => ({
          product: {
            ...item.product,
          ...
          variant: item.product.variants.find(v => v.isDefault) || item.product.variants[0],
          quantity: item.quantity,
        }));
        setSelectedItems(items);

        // Calculate discount type and value from existing pricing
        const regularTotal = bundleData._stats.regularTotal;
        const discountedPrice = bundleData._stats.discountedPrice;

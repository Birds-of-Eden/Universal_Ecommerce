    ...
    <div className="space-y-6 p-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[2fr_1fr]">
        ...
        <div className="space-y-6">
          ...
          <Card><CardHeader><CardTitle>Next Action</CardTitle></CardHeader><CardContent className="space-y-3"><ScmSectionHeader title={supplierReturn.status} subtitle="This panel keeps approval, dispatch, and credit posting actions on the single return workspace." />{actionButtons.length === 0 ? <p className="text-sm text-muted-foreground">No direct workflow action is available for your current permissions.</p> : <div className="space-y-2">{actionButtons.map((button) => <Button key={button.action} className="w-full justify-start" variant={button.action === "cancel" ? "outline" : "default"} onClick={() => void runAction(button.action)} disabled={saving}>{button.label}</Button>)}</div>}</CardContent></Card>
        </div>
      </div>
    </div>
  );

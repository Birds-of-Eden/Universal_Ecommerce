...
export default function WarehouseDashboardPage() {
  ...
  const fetchDashboard = useCallback(
    async (nextWarehouseId?: string, showRefresh = false) => {
      try {
        ...
        const resolvedWarehouseId = nextWarehouseId ?? warehouseId;
        ...
        setData(payload);
        if (!nextWarehouseId && payload?.selectedWarehouseId) {
          setWarehouseId(String(payload.selectedWarehouseId));
        }
      } catch (fetchError) {
        setError(
  ...
  const selectedWarehouseLabel = useMemo(() => {
    if (!data?.selectedWarehouseId) return "All assigned warehouses";
    const selected = data.warehouses.find((warehouse) => warehouse.id === data.selectedWarehouseId);
    return selected ? `${selected.name} (${selected.code})` : "Assigned warehouse";
  }, [data]);
      ...
      <div className="min-h-screen p-6">
        <div className="flex h-80 items-center justify-center">
          <div className="flex items-center gap-3 rounded-2xl border bg-card px-6 py-4 shadow-sm">
            ...
            <span className="text-sm text-muted-foreground">Loading warehouse dashboard...</span>
          </div>
        </div>
      </div>
    );
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
                Monitor assigned warehouse activity, shipment queue, and stock health from a
                single scoped workspace.
              </p>
            </div>
            ...
            <div className="flex flex-col gap-3 sm:flex-row">
              <select
                ...
                onChange={(event) => {
                  ...
                  void fetchDashboard(nextValue);
                }}
                className="min-w-[240px] rounded-2xl border border-border bg-background px-4 py-3 text-sm text-foreground outline-none transition focus:border-primary"
              >
        ...
        <section className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
          <article className="rounded-3xl border bg-card p-5 shadow-sm">
            <div className="flex items-center justify-between gap-3">
              <div>
                <h2 className="text-lg font-semibold text-foreground">Warehouse Coverage</h2>
            ...
            <div className="mt-4 space-y-3">
              {data?.warehouseCards.length ? (
                data.warehouseCards.map((card) => (
                  <div
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-base font-semibold text-foreground">{card.totalUnits}</p>
                    </div>
                    <div>
                      <p className="text-xs uppercase tracking-[0.18em] text-muted-foreground">
                        Reserved
                      </p>
                      <p className="mt-1 text-base font-semibold text-foreground">{card.reservedUnits}</p>
                    </div>
                    <div>
                      <p className="text-xs uppercase tracking-[0.18em] text-muted-foreground">
                        Shipments
            ...
            <div className="mt-4 space-y-3">
              {data?.lowStock.length ? (
                data.lowStock.map((item) => (
                  <div key={`${item.warehouseId}:${item.variantId}`} className="rounded-2xl border bg-background p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div>
        ...
        <section className="grid gap-6 xl:grid-cols-2">
          ...
          <article className="rounded-3xl border bg-card p-5 shadow-sm">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-lg font-semibold text-foreground">Recent Inventory Activity</h2>
            </div>
            <div className="mt-4 space-y-3">
